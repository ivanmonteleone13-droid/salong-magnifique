...
function Stars({ rating }: { rating: number }) {
  return (
    <div className="flex gap-0.5 text-[#D4AF37]" aria-label={`${rating} av 5 stjärnor`}>
...
export default function ReviewsCarousel() {
  ...
  return (
    <section id="omdomen" className="bg-[#4A1942] py-20 text-white">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-sm font-semibold uppercase tracking-wider text-[#D4AF37]">Omdömen</p>
          ...
          <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#D4AF37]/20 px-4 py-2 text-sm font-semibold text-[#D4AF37]">
            ...
            <div className="flex gap-2" role="tablist" aria-label="Välj omdöme">
              {business.testimonials.map((_, i) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full transition ${i === current ? "bg-[#D4AF37] w-6" : "bg-white/30"}`}
        ...
        <div className="mt-10 text-center">
          <a
            ...
            className="inline-flex items-center justify-center rounded-full bg-[#D4AF37] px-8 py-3 text-sm font-semibold text-[#2D0F28] transition hover:bg-[#b8921f]"
          >
