...
export default function Hero() {
  ...
  return (
    <section
      id="hem"
      className="relative overflow-hidden bg-gradient-to-br from-[#2D0F28] via-[#4A1942] to-[#4A1942]/80 text-white"
    >
      {heroImage && (
        <>
          <img
            src={heroImage}
            alt={`${business.name} — skönhetssalong på ${business.address.street}, ${business.address.city}`}
            className="absolute inset-0 h-full w-full object-cover"
            fetchPriority="high"
          />
          <div className="absolute inset-0 bg-[#2D0F28]/70" aria-hidden />
        </>
      )}
      {!heroImage && (
        <div className="absolute inset-0 opacity-20" aria-hidden>
          <div className="absolute -right-20 top-10 h-72 w-72 rounded-full bg-[#D4AF37] blur-3xl" />
      ...
      <div className="relative mx-auto grid max-w-6xl gap-10 px-4 py-20 sm:px-6 lg:grid-cols-2 lg:items-center lg:py-28">
        <div>
          <p className="mb-4 inline-flex items-center gap-2 rounded-full bg-[#D4AF37]/20 px-4 py-1.5 text-sm font-semibold text-[#D4AF37] ring-1 ring-[#D4AF37]/30">
            <span aria-hidden>★</span>
            {business.rating}/5 · {business.reviewCount}+ omdömen
          </p>
          <h1 className="text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            Skönhet & välmående
            <span className="block text-[#D4AF37]">i hjärtat av Fyrislund</span>
          </h1>
          ...
          <div className="mt-10 flex flex-col gap-4 sm:flex-row">
            <a
              ...
              className="inline-flex items-center justify-center rounded-full bg-[#D4AF37] px-8 py-4 text-base font-semibold text-[#2D0F28] shadow-lg transition hover:opacity-90"
            >
